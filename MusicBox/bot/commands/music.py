...
class Music(commands.Cog):
    ...
    async def _play_song(self, ctx, *, track: str):
        ...
        if audiocontroller.playlist.loop == True:
            await ctx.send("Loop is enabled! Use {}loop to disable".format(config.BOT_PREFIX)) #Loop is enabled! Use {}loop to disable
            return
    ...
    async def _shuffle(self, ctx):
        ...
        if current_guild.voice_client is None or not current_guild.voice_client.is_playing():
            await ctx.send("Очередь пуста :x:")#Queue is empty
            return

        audiocontroller.playlist.shuffle()
        await ctx.send("Очередь перемешана :twisted_rightwards_arrows:")#Shuffled queue
    ...
    async def _pause(self, ctx):
        ...
        await ctx.send("Воспроизведение приостановлено :pause_button:")#Playback Paused
    ...
    async def _stop(self, ctx):
        ...
        await ctx.send("Stopped all sessions :octagonal_sign:")#Stopped all sessions
    ...
    async def _volume(self, ctx, *args):
        ...
        try:
            ...
            await ctx.send("Ошибка: Громкость должна быть в пределах 1-100")#Error: Volume must be a number 1-100
